...
fn main() {
    ...
    cli_print(Level::Info, &format!("    
░░▀ █▀▀ █░░ █░░ █░░█ █▀▀ ░▀░ █▀▀ █░░█ █ 
░░█ █▀▀ █░░ █░░ █▄▄█ █▀▀ ▀█▀ ▀▀█ █▀▀█ ▀ 
█▄█ ▀▀▀ ▀▀▀ ▀▀▀ ▄▄▄█ ▀░░ ▀▀▀ ▀▀▀ ▀░░▀ ▄
=======================================>
v{} | swish swish🪼", VERSION));
    ...
    {
        cli_print(Level::Warning, &format!("depth ({}) might be too expensive! consider lowering if CPU usage is too high.", args.depth));
    }

    if args.depth == 0 
    {
        cli_print(Level::Error, "you cannot have a depth of 0, this leaves nothing for me to process! >:(");
        exit(0)
    }

    if args.brightness > 1.0 || args.brightness < 0.0 {
        cli_print(Level::Error, &format!("brightness ({}) must be between 0.0 and 1.0!", args.brightness));
        exit(0);
    }
    
    cli_print(Level::Info, &format!("using monitor: {}; width: {}, height: {}", args.monitor, width, height));
    ...
    let mut jelly: render::JellyRenderer =
        render::JellyRenderer::new(
            width, 
            height, 
            args.n_of_leds,
            args.depth, 
            ColorOption::new(args.brightness),
            CalculationOption::new(args.disable_color_operations));

    cli_print(Level::Info, &format!("connected to {} that has a number of {} LEDs.", args.port, args.n_of_leds));
    // reset LEDs to black.
    neobridge.set_all(RGB(0, 0, 0));
    neobridge.show();

    cli_print(Level::Info, &format!("sent reset commands to {}, assuming board works", args.port));
